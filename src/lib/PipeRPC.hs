...
instance Member' elem list (IsHead elem list) => Member elem list where
  index = index' @elem @list @(IsHead elem list)

class Member' elem (list :: [*]) (atHead :: Bool) where
  index' :: Int

instance Member' h (h ': t) 'True where
  index' = 0

instance Member' e t (IsHead e t) => Member' e (h ': t) 'False where
  index' = 1 + index' @e @t @(IsHead e t)

type family IsHead elem (list :: [*]) :: Bool where
  IsHead h (h ': _) = 'True
  IsHead h _        = 'False
